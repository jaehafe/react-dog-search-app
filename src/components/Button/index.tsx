...
import { getError, getImageResults, getReset } from '../../reducers/app';
...
function Button({ setImages, setIsLoading }: Props) {
  ...
  const dogStore = useSelector((state: RootState) => state.breedSet);
      ...
      await fetchBreedImages(breedState, numberState)
        .then((data) => {
          if (data?.status === 'success' && data?.message?.length) {
            ...
            setIsLoading(false);
            dispatch(getImageResults(data?.message.length));
          }
        })
        .catch((error) => {
          console.error(error);
